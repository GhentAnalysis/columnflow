...
```bash
law run cf.<Task> <flags> --print-output 0
```

`--print-output DEPTH`: `0` shows only the target task; `-1` walks the full upstream dependency tree. Each output line is marked **existent** (present) or **absent** (missing). 

# Opening a produced Parquet file

```bash
cf_inspect <path-to-parquet-file>
```
