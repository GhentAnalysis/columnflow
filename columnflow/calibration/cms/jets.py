...
def jec(
    ...
    *get_jec_config* can be adapted in a subclass in case it is stored differently in the config.
    ...
    if self.propagate_met:
        # Check and handle non-finite MET values
        inf_met = ~np.isfinite(events[self.raw_met_name].pt)
        def protect(arr):
            return ak.where(inf_met, 0., arr)
        if ak.any(inf_met):
            print(f"Found infinite MET values, replacing with 0.")
        events = set_ak_column_f32(events, f"{self.raw_met_name}.pt", protect(events[self.raw_met_name].pt))
        events = set_ak_column_f32(events, f"{self.raw_met_name}.phi", protect(events[self.raw_met_name].phi))
...
def jec_setup(self: Calibrator, reqs: dict, inputs: dict, reader_targets: InsertableDict) -> None:
    ...
    def make_jme_keys(names, jec=jec_cfg, is_data=self.dataset_inst.is_data):
        if is_data:
            ...
            if jec.campaign.startswith("Summer22"):
                prefix = f"{jec.campaign}_{jec_era}"
            else:
                prefix = f"{jec.campaign}"
        return [
            f"{prefix}_{jec.version}_DATA_{name}_{jec.jet_type}" # TODO for 2024 currently no era division
