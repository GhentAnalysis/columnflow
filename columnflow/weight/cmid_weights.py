...
def cmid_weights(self: WeightProducer, events: ak.Array, **kwargs) -> ak.Array:
    """
    ...
    shifts that the produced event weight depends on.
    """
    ...
    if self.dataset_inst.is_mc and len(events):
        ...
        for column in self.dataset_inst.x.dataset_weights:
            weight = weight * Route(column).apply(events)
    return events, weight


@cmid_weights.init
def cmid_weights_init(self: WeightProducer) -> None:
    if not getattr(self, "dataset_inst", None):
        return

    weight_columns = set()
    # add used weight columns and declare shifts that the produced event weight depends on
    if self.config_inst.has_aux("event_weights") and self.dataset_inst.is_mc:
    ...
    if self.dataset_inst.has_aux("event_weights") and self.dataset_inst.is_mc:
    ...
    if self.dataset_inst.has_aux("dataset_weights"):
        weight_columns |= {Route(column) for column in self.dataset_inst.x.dataset_weights}
        for shift_insts in self.dataset_inst.x.dataset_weights.values():
            self.shifts |= {shift_inst.name for shift_inst in shift_insts}
